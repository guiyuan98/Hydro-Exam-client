...
- 源码文件操作：支持打开本地 `.cpp/.cc/.cxx/.hpp` 文件、保存、另存为，适合直接管理比赛源码。
- OI 数据包导入：支持导入官方 zip 数据包，自动解压并识别 `.in` 与 `.out/.ans`，批量生成本地测试点。
...
- `Ctrl+O`：打开本地源码文件。
- `Ctrl+S`：保存当前源码文件。
- `Ctrl+Shift+S`：当前源码另存为。
- 本地编辑器支持 `()`、`[]`、`{}`、`""`、`''` 自动补全，`Enter` 自动缩进，`Tab`/`Shift+Tab` 缩进或取消缩进，`Ctrl+/` 快捷注释或取消注释。

本地测试数据导入：

- 点击编辑器工具栏中的“导入数据包 zip”。
- 选择 OI 官方提供的数据包，例如包含 `1.in`、`1.out`、`2.in`、`2.out` 的 zip。
- 客户端会自动解压并按文件名配对输入/输出，生成测试点。
- 如果输出文件是 `.ans`，也会作为期望输出识别。
